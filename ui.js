...
.info-card{display:flex;gap:12px;align-items:flex-start;margin-bottom:13px;padding:15px 17px;border:1px solid #cde0f5;border-radius:14px;background:#edf6ff;color:#24476d}.info-card span{font-size:13px}.match-card{padding:17px;margin-bottom:12px}.match-card h3{margin:0 0 4px}.match-line{display:grid;grid-template-columns:1fr auto;gap:12px;align-items:center;padding:13px 0;border-top:1px solid var(--line)}.score{display:grid;place-items:center;width:54px;height:54px;border-radius:50%;background:#eaf3ff;color:var(--blue);font-weight:900}.task-done{opacity:.58}.task-done .row-main b{text-decoration:line-through}.overdue{color:var(--danger)!important;font-weight:700}
...
function selectField(label,name,items,current,wide,placeholder){return'<label class="'+(wide?"wide":"")+'">'+esc(label)+'<select name="'+esc(name)+'">'+optionList(items,current,placeholder)+'</select></label>'}
...
function openForm(kind,id){
  ...
  if(kind==="demand"){var customers=S.customers.map(function(item){return{value:item.id,label:item.name+(item.phone?" · "+item.phone:"")}});html=selectField("Müşteri","customer_id",customers,value(record,"customer_id"),false)+selectField("İşlem Türü","type",["Satılık","Kiralık"],value(record,"type")||"Satılık",false,false)+field("İlçe","district","text",value(record,"district"))+field("Mahalle","neighborhood","text",value(record,"neighborhood"))+field("Min. Bütçe","budget_min","number",value(record,"budget_min"))+field("Maks. Bütçe","budget_max","number",value(record,"budget_max"))+field("Oda Sayısı","rooms","text",value(record,"rooms"),false,false,"3+1")+selectField("Taşınmaz Türü","property_type",["Daire","Villa","Arsa","Dükkan","Ofis","Bina","Tarla","Diğer"],value(record,"property_type")||"Daire",false,false)+field("Min. Net m²","net_min","number",value(record,"net_min"))+field("Maks. Net m²","net_max","number",value(record,"net_max"))+selectField("Durum","status",["Aktif","Beklemede","Sonuçlandı"],value(record,"status")||"Aktif",false,false)+textareaField("Notlar","notes",value(record,"notes"))}
...
function bindEvents(){
  ...
  el("modal").addEventListener("click",function(event){if(event.target===el("modal"))closeModal()});
